...
describe("Users Crud Test", () => {
    ...
    describe("GET /v1/users", () => {
        it("should response 200 (Ok) and successfully retrive user data", async () => {
            ...
            if (bodyResult.message === "Users Retrieved") {
                for (const item of bodyResult.data) {
                    expect(item).toEqual({
                        email: expect.stringMatching(emailRegex) as never,
                        id: expect.stringMatching(UUIDRegex) as never,
                        password: expect.any(String) as never,
                        username: expect.any(String) as never,
                        role: expect.stringMatching(/admin|user/u) as never,
                        fullname: expect.any(String) as never,
                        emailVerified: expect.any(Boolean) as never
                    });
                }
            }
        });
    ...
    describe("POST /v1/users", () => {
        ...
        it("should response 400 (Bad Request) when field is provided with invalid value", async () => {
            ...
            expect(bodyResult.data).toEqual({
                on: "body",
                summary: expect.any(Array) as never
            });
        });
